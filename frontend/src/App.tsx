...
import { ApiError, DEMO_BASE, ENGINE_BASE, api } from "./api/client";
import type { CheckResult, DemoMode, Finding, Scan } from "./api/types";
import { FindingsList } from "./components/FindingsList";
import { SCAN_STEPS, ScanProgress } from "./components/ScanProgress";
import { SummaryCards } from "./components/SummaryCards";
import { Card, ModeBadge, Spinner, StatusDot } from "./components/ui";
    ...
    <div className="min-h-full">
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-3 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Evidence-Based API Authorization Testing</p>
            </div>
          </div>
          <div className="ml-auto flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-md border border-line bg-surface-800 px-2.5 py-1 text-[11px] font-semibold tracking-wide text-slate-300">
              <StatusDot ok={demoUp} /> Local Demo API
            </span>
            <ModeBadge mode={mode} />
          </div>
        </div>
      ...
      <main className="mx-auto max-w-7xl space-y-5 px-6 py-6">
        ...
        <footer className="pb-4 text-center text-[11px] text-slate-600">
          API Sentinel · educational tool · scans only local demo targets ({ENGINE_BASE} → {DEMO_BASE})
        </footer>
      </main>
    </div>
  );
